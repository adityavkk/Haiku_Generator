...
Dict.prototype.findRandomMatch = function(pattern) {
	var words = Object.keys(this);
	words = words.shuffle();
	return pattern.map(numOfSyllables => {
		for (var i = 0; i < words.length; i++) {
			if(this[words[i]] == numOfSyllables)
				return words[i];
		}
	}).join(' ');
}
...
function createHaiku(pattern, cmuDictPath, textPathsArr) {
	var haiku = '', syllableLookup = syllableLookupGenSync(cmuDictPath), numOfTextsToParse = Math.min(pattern.length, textPathsArr.length), parsedTexts = [];
	var randomIndices = arrayGen(0, textPathsArr.length - 1).shuffle().slice(0, numOfTextsToParse);
	randomIndices.forEach(e => {
		parsedTexts.push(parseTextSync(textPathsArr[e], syllableLookup));
	});
	return pattern.map((line, i) => parsedTexts.next(i).findRandomMatch(line)).join('\n');
}
...
console.log(createHaiku([[5,5],[5,5],[5,5]], './cmudict.txt', ['./trial.txt', './metamorphosis.txt']));